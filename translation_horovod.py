...
def create_model(
    ...
    optimizer = tf.train.AdamOptimizer(
        learning_rate=0.001 * hvd.size(), beta1=0.9, beta2=0.98, epsilon=1e-9)
...
def run(
    ...
    # prevent other workers from corrupting the checkpoints
    model_dir_ = model_dir if hvd.rank() == 0 else None

    config = tf.estimator.RunConfig(
        model_dir=model_dir_, session_config=session_config)
